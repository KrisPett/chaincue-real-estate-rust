...
#[get("/account")]
pub async fn get_account_page(data: web::Data<AppState>) -> Result<HttpResponse, Error> {
    log::info!("get_account_page");
    let dbc = Arc::new(data.dbc.clone());
    let dto = build_dto(&dbc, |_builder| async { Ok(()) }).await?;
    Ok(HttpResponse::Ok().json(dto))
}

async fn build_dto<F, Fut>(dbc: &Arc<DatabaseConnection>, additional_processing: F) -> Result<AccountPageDTO, Error>
    where
...
fn to_home_page_dto(_dto_builder: DTOBuilder) -> AccountPageDTO {
    AccountPageDTO {
        id: String::from("dtoBuilder.id")
    }
}
